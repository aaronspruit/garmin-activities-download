...
5. **Keep the notes in agent-memory current.** The `agent-memory` MCP server holds every note. This repository writes to the store `garmin-activities-download`, which is a note store on that server and not a git branch. One subject gets one store, so a note about the homek8 cluster, or about any other subject, goes to the store of that subject and never here. Do not write to the local memory directory. This store does not load itself into context, so every recall is a search that you run.
