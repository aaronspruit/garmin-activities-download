...
1. **[src/config.py](src/config.py)** — `load_config()` builds a `Config` dataclass from env vars, with Docker-secret fallback: `_read_secret()` reads `/run/secrets/<name lowercased>` first, then the env var. `DOWNLOAD_FORMATS` parses into a list of `DownloadTarget(format, subfolder)` — each comma-separated entry is `FORMAT` (no subfolder) or `FORMAT:subfolder`. `DownloadTarget.path` is the destination relative to `output_dir`: `FORMAT` or `FORMAT/subfolder`, so a subfolder is always nested under its format's folder and formats never share a folder. Formats are validated against `{FIT, GPX, TCX}`; subfolders must be a single safe path component. Both raise `ValueError`. A repeated `(format, subfolder)` pair is silently deduplicated, keeping first-occurrence order.
...
3. **[src/downloader.py](src/downloader.py)** — `download_new_activities()` fetches activities in the `days_back` window and downloads each configured target. Each `activityId` from the Garmin response is checked by `_is_safe_activity_id()` (ASCII alphanumeric only) before it reaches a path; a failure raises `UnsafeActivityIdError`, which `main.py` maps to exit code 3. **Dedup is filesystem-based**: an activity/target is skipped if `<output_dir>/<target.path>/<activityId>.<ext>` already exists — there is no database or manifest. Behavior per format is driven by the `FORMAT_SPECS` table. Targets are grouped by format so a format wanted in several folders costs one Garmin download per activity, written to every folder still missing it. FIT is special: Garmin returns it as an `ORIGINAL`-format zip, so `_extract_fit_bytes()` pulls the first `.fit` member out before writing. A `download_delay` (default 1s) between downloads guards against rate limits.
...
Files are written into one folder per format — `data/FIT/`, `data/GPX/`, `data/TCX/` — with an optional single level of target subfolder below it (`data/FIT/archive/`). Earlier versions wrote GPX flat into `data/`. Because dedup is path-based, flat legacy files are not recognized as already-downloaded — this is a known breaking change noted in the README.

## Configuration

All config is env-var driven (see the table in [README.md](README.md)). Key vars: `GARMIN_EMAIL`, `GARMIN_PASSWORD` (both secret-aware), `DAYS_BACK` (default 7), `GARMINTOKENS` (default `/app/tokens`), `OUTPUT_DIR` (default `/app/data`), `DOWNLOAD_FORMATS` (default `FIT`, accepts `FORMAT[:subfolder]` entries and repeated formats). Docker Compose mounts `./data` and `./tokens` as volumes to persist across the run-once lifecycle.
