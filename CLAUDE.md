...
1. **Write all documentation in Simplified Technical English (ASD-STE100).** This covers the README, this file, comments and docstrings, commit messages, pull request titles and bodies, release notes, and messages the operator reads. Apply the rules before you draft, not after. The [`simple-english` skill](https://github.com/AminBlg/SimpleEnglish) holds them.
...
- `{format}` and `{tracker}` resolve at load time, because both are known there. The tracker comes from the variable name. The downloader therefore sees concrete paths only. `_render_folder()` substitutes them with `str.replace`, not `str.format_map`: this grammar has no escape, so `{{` must stay an error. Any brace left after the substitution raises.
...
`CLAUDE.local.md` is not in git. Use it for instructions that belong to you and not to the repository.
...
### Keep the notes in agent memory current

This rule has a condition. If the session holds a memory tool, obey the rule. A
memory tool is an MCP server that reads and writes notes, such as
`basic-memory`. If the session holds no memory tool, skip this rule.

One subject gets one project. The project for a repository has the name of that
repository, without the owner. A note about another subject, such as a cluster
or a service, goes to the project of that subject. The project `shared` holds a
fact that no repository owns.

1. Search before you read files, and before you plan the change. Read each note
   that the search returns.
2. Write notes during the session, and not only at the end. After each decision,
   each finding and each piece of work, write it to the project.
3. Update the note that covers the subject. Do not add a second note for it.
4. Record the outcome before you finish: the pull request number, the branch,
   whether the change is applied, and what stays open.

Record a decision and its reason, a constraint that the code does not state, and
a cause that took effort to find. Do not record the transcript of a task, or a
fact that the code or the git history already holds.

CAUTION: A call that omits `project` writes to the default project, and it
reports no error. A search that supplies `project` ignores
`search_all_projects`, and it also reports no error. Supply `project` to write.
Omit it to search every project.
<!-- harness:rules:end -->
